...
def index():
    ...
    return render_template('index.html') # Fixed redirect loop
...
def login():
    if request.method == 'POST':
        ...
        try:
            db, cursor = get_db()
            # Fetch user ONLY by username first
...
def register():
    if request.method == 'POST':
        ...
        # HASH THE PASSWORD BEFORE SAVING TO SESSION
        ...
        if session.get('otp_type') == "register":
            ...
            cursor.execute(
                ...
                (data['fullname'], data['username'], data['email'], data['contact'],
                 ...
                 data['id_file_path'], data['password']) # Inserts hashed password
            )
            db.commit()
...
def reset_password():
    if request.method == 'POST':
        ...
        # Hash new password before updating DB
...
def send_ride_notifications():
    try:
        # Load env vars safely for the background thread DB config
